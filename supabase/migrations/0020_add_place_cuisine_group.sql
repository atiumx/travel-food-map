-- Batch G2: add_place RPC — accept cuisine_group + handle taipei mapping
...
declare
  ...
  v_tabelog_url  text;
  v_day_tag      smallint;
  v_cuisine_group public.cuisine_group;
begin
  ...
  elsif p_trip_area_slug = 'taipei' then
  ...
  -- G2: accept cuisine_group (fallback 'other')
  begin
    v_cuisine_group := coalesce(nullif(p_place->>'cuisine_group',''), 'other')::public.cuisine_group;
  exception when invalid_text_representation then
    v_cuisine_group := 'other';
  end;

  insert into public.places (
    trip_area_id, trip_id, name, category, cuisine_group, region,
  ...
  values (p_display_name, 'add_place', 'places', v_place_id,
    jsonb_build_object('invite_code_id', v_invite_id, 'trip_area_slug', p_trip_area_slug, 'place_name', v_name, 'day_tag', v_day_tag, 'cuisine_group', v_cuisine_group));
